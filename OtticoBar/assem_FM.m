function [Kmat0,Jmat0,Jmat1,Jmat2,uvet,rvet,mode,tvet]=assem_FM(geom,mesh,mode,uvet,v0_dd)
%
% define constants
%'qo4er intap', keyboard
% Carico dati Vallone per tau
global coM coX coE coMh
%
s_LoadConstants
...
for ic=1:nc; ii=(contact==ic); % loop on contacts
    ...
    uvet(qq+1)=v_ls;
    if(mode.nflg), elec=abs(uvet(nn+(1:nn))); end % electron concentration
    if(mode.pflg), hole=abs(uvet(pp+(1:nn))); end % hole concentration
    if(mode.oflg)
    ...
    if(mode.nflg), mode.EFn(iq) = mode.ecb(iq) + Vt(iq).*log(elec(iq)./mesh.Nc(iq)); end
    if(mode.pflg), mode.EFp(iq) = mode.evb(iq) - Vt(iq).*log(hole(iq)./mesh.Nv(iq)); end
    %
    if((isfield(mode,'stats'))&&(strcmp(mode.stats,'Fermi'))) % Fermi ---------
        ...
        if(mode.nflg),
            ...
            tmp = elec./Nc;
            mode.EFn = mode.ecb + Vt.*invferdr(tmp,mode.tolinvferdr); end
        ...
        if(mode.pflg),
            ...
            tmp = hole./Nv;
            mode.EFp = mode.evb - Vt.*invferdr(tmp,mode.tolinvferdr); end
    ...
    if(not(mode.firstrun)) % At equilibrium, R=0
        for indGR=1:length(mode.GR)
            ...
            n  =  elec(iq); p  =  hole(iq);
            np = (n.*p-gamman(iq).*gammap(iq).*nint(iq).^2);
            dnp_n = (p-dgamman(iq).*gammap(iq).*mesh.nint(iq).^2);
            dnp_p = (n-dgammap(iq).*gamman(iq).*mesh.nint(iq).^2);
            if strcmp(mode.GR{indGR},'SRH')==1
                % SRH recombination model
                n1 = gamman(iq).*nint(iq).*exp(mesh.Etrap(iq)./Vt(iq));
                p1 = gammap(iq).*nint(iq).*exp(-mesh.Etrap(iq)./Vt(iq));
                denSRH = mesh.taup(iq).*(n+n1)+mesh.taun(iq).*(p+p1);
                RSRH(iq) = np./denSRH;
                R(iq) = R(iq) + RSRH(iq);
                dRn(iq) = dRn(iq) + ((p-dgamman(iq).*gammap(iq).*nint(iq).^2).*denSRH - ...
                    mesh.taup(iq).*(1+dgamman(iq).*n1).*np)./(denSRH.^2);
                dRp(iq) = dRp(iq) + ((n-dgammap(iq).*gamman(iq).*nint(iq).^2).*denSRH - ...
            ...
            elseif (strcmp(mode.GR{indGR},'BTBT')==1)
                ...
                %
                Vinterp=mode.EFp(iTJp)-mode.EFn(iTJn);
                ...
                dVinterp_dphip(IBTJ) = -1; % derivata rispetto a EFp (-1 per colpa di Ev = -q*phi + puffi)
                dVinterp_dphin(IBTJ) = +1; % derivata rispetto a EFn (-1 per colpa di Ev = -q*phi + puffi, altro -1 perché ha segno -)
                ...
                %                         load cBTBTdouble % no T dependence
                load Interp_Fit_AlGaAs_Temp     % Main_3, T dependence
                
                [~,iT]=min(abs(T_vet-mean(mesh.T(iTJp:iTJn))));
                cT=c(iT,:); % fitting coefficient at a given temperature
                ...
                % fit cubico
                VV = Vinterp.^[pDeg:-1:0];
                dVV = Vinterp.^[pDeg-1:-1:0];
                
                II = zeros(1,mesh.nn);
                II(IBTJ) = 1/LTJ;  % should be inserted ONLY in the lateral nodes of the BTJ "relief"
                II = II.*(-VV*cT.'/qel);
                ...
                dRnTJ = dII.*dVinterp_dn;
                dRpTJ = dII.*dVinterp_dp;
                dRphin = dII.*dVinterp_dphin;
                dRphip = dII.*dVinterp_dphip;
                %
                % GBTBT derivatives assembled
                %
                % Assembling in electron continuity equation
                % Recombination term
                MM = qel.*[Se1.*GBTBT(in1) Se2.*GBTBT(in2) Se3.*GBTBT(in3)];
                tvet = tvet + sparse(ijs+nn,1,MM(mask_ijs),neq,1);
                %
                for iBTBT=1:length(ITJp_vet)
                    % Derivative of recombination term w.r.t. electrons
                    MM = qel.*[Se1.*dRnTJ(in1) Se2.*dRnTJ(in2) Se3.*dRnTJ(in3)];
                    Jmat0 = Jmat0 + sparse(ijs+nn,ITJp_vet(iBTBT)+nn,MM(mask_ijs),neq,neq);
                    % Derivative of recombination term w.r.t. holes
                    MM = qel.*[Se1.*dRpTJ(in1) Se2.*dRpTJ(in2) Se3.*dRpTJ(in3)];
                    Jmat0 = Jmat0 + sparse(ijs+nn,ITJn_vet(iBTBT)+pp,MM(mask_ijs),neq,neq);
                    % Derivative of recombination term w.r.t. phi (left node)
                    MM = qel.*[Se1.*dRphin(in1) Se2.*dRphin(in2) Se3.*dRphin(in3)];
                    Jmat0 = Jmat0 + sparse(ijs+nn,ITJn_vet(iBTBT),MM(mask_ijs),neq,neq);
                    % Derivative of recombination term w.r.t. phi (left node)
                    MM = qel.*[Se1.*dRphip(in1) Se2.*dRphip(in2) Se3.*dRphip(in3)];
                    Jmat0 = Jmat0 + sparse(ijs+nn,ITJp_vet(iBTBT),MM(mask_ijs),neq,neq);
                end
                %
                ...
                for iBTBT=1:length(ITJp_vet)
                    % Derivative of recombination term w.r.t. holes
                    MM = qel.*[Se1.*dRpTJ(in1) Se2.*dRpTJ(in2) Se3.*dRpTJ(in3)];
                    Jmat0 = Jmat0 + sparse(ijs+pp,ITJp_vet(iBTBT)+pp,MM(mask_ijs),neq,neq);
                    % Derivative of recombination term w.r.t. electrons
                    MM = qel.*[Se1.*dRnTJ(in1) Se2.*dRnTJ(in2) Se3.*dRnTJ(in3)];
                    Jmat0 = Jmat0 + sparse(ijs+pp,ITJn_vet(iBTBT)+nn,MM(mask_ijs),neq,neq);
                    % Derivative of recombination term w.r.t. phi (left node)
                    MM = qel.*[Se1.*dRphin(in1) Se2.*dRphin(in2) Se3.*dRphin(in3)];
                    Jmat0 = Jmat0 + sparse(ijs+pp,ITJn_vet(iBTBT),MM(mask_ijs),neq,neq);
                    % Derivative of recombination term w.r.t. phi (left node)
                    MM = qel.*[Se1.*dRphip(in1) Se2.*dRphip(in2) Se3.*dRphip(in3)];
                    Jmat0 = Jmat0 + sparse(ijs+pp,ITJp_vet(iBTBT),MM(mask_ijs),neq,neq);
                    %
                end
            end
        end
    ...
    % =============================================================================================100
    % doping dynamics in current continuity eqs.
    ...
    MM=qel.*[Se1.*dop_dp(in1) Se2.*dop_dp(in2) Se3.*dop_dp(in3)];
    tvet=sparse(ijr,1,-MM(mask_ijr),neq,1);
    %
    MM=qel.*[Se1.*dop_am(in1) Se2.*dop_am(in2) Se3.*dop_am(in3)];
    ...
    if(mode.firstrun) % -------------------------------------------------------
        ...
        %
        MM=qel.*[Se1.*pF(in1) Se2.*pF(in2) Se3.*pF(in3)];
        tvet=tvet+sparse(ijr,1,-MM(mask_ijr),neq,1);
        %
        MM=qel.*[Se1.*nF(in1) Se2.*nF(in2) Se3.*nF(in3)];
        tvet=tvet+sparse(ijr,1, MM(mask_ijr),neq,1);
        %
        MM=qel.*[dpF(in1).*Se1 dpF(in2).*Se2 dpF(in3).*Se3];
        Jmat0=Jmat0+sparse(ijr,ijr,-MM(mask_ijr),neq,neq);
        %
        MM=qel.*[dnF(in1).*Se1 dnF(in2).*Se2 dnF(in3).*Se3];
        Jmat0=Jmat0+sparse(ijr,ijr, MM(mask_ijr),neq,neq);
        %
        if((isfield(mode,'ionization'))&&(strcmp(mode.ionization,'Incomplete')))
            MM=qel.*[ddop_dp(in1).*dnF(in1).*Se1 ddop_dp(in2).*dnF(in2).*Se2 ddop_dp(in3).*dnF(in3).*Se3];
            Jmat0=Jmat0+sparse(ijr,ijr,-MM(mask_ijr),neq,neq);
            %
            MM=qel.*[ddop_am(in1).*dpF(in1).*Se1 ddop_am(in2).*dpF(in2).*Se2 ddop_am(in3).*dpF(in3).*Se3];
            Jmat0=Jmat0+sparse(ijr,ijr,MM(mask_ijr),neq,neq); end
        %
        mode.elec(mesh.iq) = abs(nF(mesh.iq));
        mode.hole(mesh.iq) = abs(pF(mesh.iq)); end % -----------------------------------
    %
    % incomplete ionization
    if((isfield(mode,'ionization'))&&(strcmp(mode.ionization,'Incomplete')))
        if(mode.nflg),
            MM=qel.*[ddop_dp(in1).*Se1 ddop_dp(in2).*Se2 ddop_dp(in3).*Se3];
            Jmat0=Jmat0+sparse(ijr,ijr+nn,-MM(mask_ijr),neq,neq); end
        if(mode.pflg),
            MM=qel.*[ddop_am(in1).*Se1 ddop_am(in2).*Se2 ddop_am(in3).*Se3];
            Jmat0=Jmat0+sparse(ijr,ijr+pp,MM(mask_ijr),neq,neq); end
    end
    % =============================================================================================100
    % elec charge in Poisson rhs
    if(mode.nflg), MM=qel.*[Se1 Se2 Se3];
        Kmat0 = Kmat0 + sparse(ijr,ijr+nn,MM(mask_ijr),neq,neq); end
    % =============================================================================================100
    % hole charge in Poisson rhs
    if(mode.pflg), MM=-qel.*[Se1 Se2 Se3];
        Kmat0 = Kmat0 + sparse(ijr,ijr+pp,MM(mask_ijr),neq,neq); end
    % #############################################################################################100
    % =============================================================================================100
    ...
    if(mode.oflg)
        ...
        for indQW=1:NQW
            ...
            %
            SeQW1=zeros(1,mesh.nt); SeQW2=SeQW1; SeQW3=SeQW1;
            ...
            %
            xQW = mesh.node(1,inQW); yQW = mesh.node(2,inQW);
            ...
            Vt2D = Vt(inQW);
            %
            tauscatn = mesh.tauscatnMQW{indQW}; % capture time, s
            tauscatp = mesh.tauscatpMQW{indQW}; % capture time, s
            ...
            %        'prima di calcolo EFn2D',keyboard
            ...
            dn2Dm_EFn2DPois(:,1:length(dn2Dm_EFn2D))=dn2Dm_EFn2D;
            ...
            MM = qel.*[Se1.*N2D(in1) Se2.*N2D(in2) Se3.*N2D(in3)];
            tvet = tvet + sparse(ijr,1,MM(mask_ijr),neq,1);
            MM = qel.*[Se1.*dN2D_n2D(in1) Se2.*dN2D_n2D(in2) Se3.*dN2D_n2D(in3)];
            ...
            p2DmPois(:,1:length(p2Dm))=p2Dm;
            dp2Dm_EFp2DPois(:,1:length(dp2Dm_EFp2D))=dp2Dm_EFp2D;
            ...
            MM = - qel.*[Se1.*P2D(in1) Se2.*P2D(in2) Se3.*P2D(in3)];
            tvet = tvet + sparse(ijr,1,MM(mask_ijr),neq,1);
            MM = - qel.*[Se1.*dP2D_p2D(in1) Se2.*dP2D_p2D(in2) Se3.*dP2D_p2D(in3)];
            ...
            %            Ccapn = - (1-exp((EFn2D-EFn3D(1,:))./Vt2D)).*(1-n2D./N2)./tauscatn.*nF(inQW);
            %            dN3D_Efn3D =    dnF(inQW)./Vt2D;
            %            dCcapn_N3D =  - (1-exp((EFn2D-EFn3D(1,:))./Vt2D)).*(1-n2D./N2)./tauscatn + ...
            %               - exp((EFn2D-EFn3D(1,:))./Vt2D).*(1-n2D./N2).*nF(inQW)/tauscatn./dN3D_Efn3D./Vt2D; %th der. inversa
            %            dCcapn_n2D =  + exp((EFn2D-EFn3D(1,:))./Vt2D).*(1-n2D./N2).*nF(inQW)/tauscatn./dn2D_EFn2D./Vt2D + ...
            %               + (1-exp((EFn2D-EFn3D(1,:))./Vt2D))./N2./tauscatn.*nF(inQW);
            ...
            %            tauescn=1e-8*ones(size(inQW));
            %            tauescp=1e-8*ones(size(inQW));
            %            'vpmy Tsy', keyboard
            ...
            % new
            %        Ccapn = - (1-n2D./N2)./tauscatnT.*nF(inQW);
            %        dCcapn_N3D =  - (1-n2D./N2)./tauscatnT;
            %        dCcapn_n2D =  1./N2./tauscatnT.*nF(inQW);
            ...
            MM = qel.*WQW.*[Lp1.*dCcapn_N3D(iiQW1) Lp2.*dCcapn_N3D(iiQW2)];
            Jmat0 = Jmat0 + sparse(VV+ijrQW,nn+inQW(ijrQW),MM,neq,neq);
            MM = qel.*WQW.*[Lp1.*dCcapn_n2D(iiQW1) Lp2.*dCcapn_n2D(iiQW2)];
            ...
            %             plot(mesh.node(1,IQW),mesh.node(2,IQW),'ro')
            RQW=zeros(1,nn);
            RQW(IQW)=R(IQW);
            ...
            %
            % 2D holes capture term
            
            %            Ccapp = - (1-exp((EFp3D(1,:)-EFp2D)./Vt2D)).*(1-p2D./P2)./tauscatp.*pF(inQW);
            %            dP3D_Efp3D =  - dpF(inQW)./Vt2D;
            %            dCcapp_P3D =  - (1-exp((EFp3D(1,:)-EFp2D)./Vt2D)).*(1-p2D./P2)./tauscatp + ...
            %               + exp((EFp3D(1,:)-EFp2D)./Vt2D).*(1-p2D./P2).*pF(inQW)./tauscatp./dP3D_Efp3D./Vt2D; %th der. inversa
            %            dCcapp_p2D =  - exp((EFp3D(1,:)-EFp2D)./Vt2D).*(1-p2D./P2).*pF(inQW)/tauscatp./dp2D_EFp2D./Vt2D + ...
            %               + (1-exp((EFp3D(1,:)-EFp2D)./Vt2D))./P2./tauscatp.*pF(inQW);
            ...
            % new
            %        Ccapn = - (1-n2D./N2)./tauscatnT.*nF(inQW);
            %        dN3D_Efn3D =    dnF(inQW)./Vt2D * 0;
            %        dCcapn_N3D =  - (1-n2D./N2)./tauscatnT;
            %        dCcapn_n2D =  1./N2./tauscatnT.*nF(inQW);
            
            %        Ccapp = - (1-p2D./P2)./tauscatpT.*pF(inQW);
            %        dCcapp_P3D =  - (1-p2D./P2)./tauscatpT;
            %        dCcapp_p2D =  1./P2./tauscatpT.*pF(inQW);
            % Calcolo integrale volumico del Ccapp
            MM = qel.*WQW.*[Lp1.*Ccapp(iiQW1) Lp2.*Ccapp(iiQW2)];
            ...
            MM = qel.*WQW.*[Lp1.*dCcapp_P3D(iiQW1) Lp2.*dCcapp_P3D(iiQW2)];
            Jmat0 = Jmat0 + sparse(WW+ijrQW,pp+inQW(ijrQW),MM,neq,neq);
            MM = qel.*WQW.*[Lp1.*dCcapp_p2D(iiQW1) Lp2.*dCcapp_p2D(iiQW2)];
            ...
            if(mode.AugerExpulsion==1)
                ...
                %          figure(489329),hold on,plot(mesh.node(2,:),GAuger_n3D)
            ...
            %            Campo_attivo=mode.Gamma_z/mode.Gamma_z(ceil(end/2));
            ...
            for indMode=1:nmodes
                ...
                Fattore_attivo=Campo_attivo(indQW);
                % disp('guadagno'),keyboard
                
                [g,dgE,dgH,rsp,drspE,drspH] = f_InterpGain_lin(n2D,p2D,indQW,indMode);
                
                %        [g1,dgE1,dgH1,rsp1,drspE1,drspH1] = f_InterpGain_lin(n2D,p2D,indQW,1);
                %        [g2,dgE2,dgH2,rsp2,drspE2,drspH2] = f_InterpGain_lin(n2D,p2D,indQW,2);
                ...
                Rsp = sum(diag(sparse(ijrQW,ijrQW,MM,nnQW,nnQW)));
                dRsp_nE = drspE.*E2;
                dRsp_nH = drspH.*E2;
                %
                % Assembling spontaneous emission power
                Si=Gamma_z.*mode.fPES(indMode).*Rsp;
                tvet(ss+indMode)=tvet(ss+indMode)+Si;
                %
                ...
                Rst = gE.*Pst(indMode).*Fattore_attivo*mode.fPdif(indMode)*WQW;
                ...
                dRst_n2D = Fattore_attivo*dgm_nE.*Pst(indMode).*mode.fPdif(indMode)*WQW;
                MM = qel.*[Lp1.*dRst_n2D(iiQW1) Lp2.*dRst_n2D(iiQW2)];
                Jmat0 = Jmat0+sparse(VV+ijrQW,VV+ijrQW,MM,neq,neq);
                %
                % Derivative of holes equation Rst with respect to holes
                dRst_p2D = Fattore_attivo*dgm_nH.*Pst(indMode).*mode.fPdif(indMode)*WQW;
                ...
                dRst_Pi = Fattore_attivo*gE.*mode.fPdif(indMode)*WQW;
