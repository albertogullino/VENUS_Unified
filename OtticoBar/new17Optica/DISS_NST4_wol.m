...
ks=ks+1;
 ...
 %al1=alve(iso,k);
% p=-du;
% fi1=find(g>0 & g/vg<GMAa &  p>-psog0);
  p=Gve(:,k);
  [du,iso]=sort(p);
  g=Gve(iso,k);
 al1=alve(iso,k);
 fi1=find(g>0 & g/vg<GMAa);
% keyboard
...
%      DIF=(log10(abs(Amix(fP)))-log10(abs(Amr(fP))));
      fina=find(isnan(DIF)==0 & isinf(DIF)==0);
      autdiff=sqrt(sum(DIF(fina).^2)/length(fP));
%      autdiff=sum(10.^DIF(fina))/length(fP);
      autc(ifii)=1./autdiff;
     if iWidth==1
...
%        puf=puf(end-kg0+1:end);
       ...
       if puf(end)>length(fov)
        break
       end
      fprec=fov(isol,puf)*lambda*1000;
      aprec=alv(isol,puf);
%      'cont aprec', keyboard      
      gprec=log10(gov(isol,puf));
...
%      plot(lF,abs(autc),lF,abs(autc1),lF,abs(autcp),lF(fiz),abs(autc(fiz)),'*'), pausak
      ...
      subplot(211),
       if isol>1
        ...
        alvpp(finz)=NaN;
        fovpp(finz)=NaN;
        plot(fovpp,alvpp,'--'), hold on
       end
      plot(fov(isol,ks:ki-1)*lambda*1000,alv(isol,ks:ki-1),'.-',Fint(ki)*lambda*1000,alv2(fiz),'wo','linewidth',2),
      hold on, 
      ...
      semilogy(fov(isol,ks:ki-1)*lambda*1000,gov(isol,ks:ki-1)/vgconv,'.-',Fint(ki)*lambda*1000,gv2(fiz)/vgconv,'wo','linewidth',2), 
      hold on, 
...
if iNObre==0
 if aprec0<1
  if ks1-k>3
   condibre=du>dus & difacc<dpeak0 & alv2(fiz)>aprec0;
  end
 else
  condibre=0;
 end
...
%     if length(fitup)*length(fitum)>0 | ( min(abs(aoud(isa,:)))<1 & length(find(goud(isa,:)/vg<GMAa/2))==length(Fint) )
...
nma0=s(1);
 ...
 fcov=1000*lambda;
 if ifp==-10
  fs=figure, subplot(211), plot(fcov*fou',aou','.-'), grid
...
for kmod=1:nmotr
 aivd=aou(kmod,:);
 fiv=find(isnan(aivd)==0);
 aiv=aivd(fiv);
 giv=gou(kmod,fiv);
 friv=fou(kmod,fiv);
 aiz=aiv(1:end-1).*aiv(2:end);
 fizv=find(aiz<0);
 fiz=fizv(1);
% subplot(211), plot(friv,aiv,friv(fiz),aiv(fiz),'wo')
% subplot(212), plot(friv,giv,friv(fiz),giv(fiz),'wo')
% pausak
 asti(kmod)=aiv(fiz);
 gsti(kmod)=giv(fiz);
 fsti(kmod)=friv(fiz);
...
if ifp==-10
  figure(fs), subplot(211), plot(fcov*fou',aou','.-'), grid
  subplot(212), semilogy(fcov*fou',2*gou'/vgconv,'.-'), 
end
[du,iso]=sort(gsti);
%if length(iso)>nmasce
%nmascel=iso(1:nmasce);
%else
%nmascel=iso;
%end
if ifp==-10
figure(fs)
 subplot(211), hold on, plot(fcov*fsti(nmascel),asti(nmascel),'wo')
 subplot(212), hold on, plot(fcov*fsti(nmascel),gsti(nmascel)*2/vgconv,'wo')
'dopo stim', keyboard
end
...
if isfield(Pf,'isortgain')
 isortgain=Pf.isortgain;
else
 isortgain=1;
end
if isortgain==0
...
%' guadagno pesato con M2', keyboard
%gsoM=gso;
if isofg==1
[du,iso]=sort(fso);
else
...
fso=fso(iso);
mso=M2l(iso);
%GCM=gso(1:nmasce)*2/vg;
%FSO=fso(1:nmasce);
%MSO=mso(1:nmasce);
%LSO=lambda./(1+FSO)*1000;
%fso=fso(1:end-1);
%gso=gso(1:end-1);
