%function [ParDD,ParOpt,ParMore]= Lay_dd(fileName)
...
%'qui', keyboard
    if ienab==1 & fidollaro >0
      ...
      elseif length(findstr(Nline,'Metalring'))==1
        ...
        contact.radius_e=extrac_n(Nline,'radius_e=');
%        'qui', keyboard
         fimes=findstr(Nline,'Nr_mesh=');
         contact.Nrmesh=str2num(Nline(fimes+8:end));
%        contact.Nr_i=extrac_n(Nline,'Nr_mesh=');
%        contact.Nr_i=extrac_n(Nline,'Nr_i=');
%        contact.Nr_m=extrac_n(Nline,'Nr_m=');
%        contact.Nr_e=extrac_n(Nline,'Nr_e=');
...
%         'doe', keyboard
       ...
       otherwise
        ...
        if fi==1
         ...
         DDmesh(cl,1)=str2num(Word(7:end));
%         'lay4 DD', keyboard
         clear Word
        else
