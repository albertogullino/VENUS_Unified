...
%     disp(' for: istr, istrc'), [istr istrc], pausak
    ...
    if igau==4 & sT>0
     isomv=isomv+Li(istr);
     if fmlsi(istr,2)>0
      fmulstrato=fmlsi(istr,2);
      fmul_prec=fmulstrato;
     else
      fmulstrato=1;
      if fmul_prec*(1-itutmir)>1
       isomv=isomvero;
%       'ver mul', keyboard
      end
      fmul_prec=fmulstrato;
     end
     isomvero=isomvero+Li(istr)*fmulstrato;
     [du,izi]=min(abs(isomv-zedis(2:length(zedis))));
%     [du,izi]=min(abs(isomv-zedis));
     DelT_z0=Tdis(end,izi);
%          isomv, isomvero, zedis(izi),     pausak
%     'Delz ', keyboard
