...
%disp(' attenzione !!!!!!!!!!!: isoga = 1 in dissus.m '), keyboard
ierr=0;
dus=.2;
if ifp>=-1
 icontr=1;
...
if min(sga)>1
 if isoga==1
  ...
  clear frv
   for kso=1:sga(1)
    aud=aou(kso,:);
    aup=aud(1:end-1).*aud(2:end);
    fi0=find(aup<0);
    if length(fi0)>0
     puf=fi0(1)+[0 1];
     co=polyfit(Fint(puf),aud(puf),1);
     frv(kso)=roots(co);
    else
     frv(kso)=100;
    end
...
%figure, plot(Fint,aoud(isa,:)), pausak
...
%  disp('key'),  keyboard
  ...
  if saou==0
   figure,
   subplot(121), plot(Fint,alvet')
   subplot(122), semilogy(Fint,Gvet')
...
% modifica 2017 may
  if min(size(gou))>1
   Gmean=mean(gou,2);
   fidafare=find(Gmean<min(Gmean)*1.5);
  else
   fidafare=1;
  end
 elseif isolut==0
