...
pograp=[220   118   651   843];
pogram=[220   118   651   843];
if exist('SPP')==0
 SPP=0;
end
if ifp==-10
% 'entro fie_new', keyboard
end
% 'entro fie_new', keyboard
...
%iLP=iLPr;
iint=1;
axlim=1.5*alim*180/pi*rr;  %1.5 fattore arbitrario
if axlim>40
 axlim=40;
end
ibaru=1;
...
%'prima calc_new', keyboard
 calc_new
% 'dopo calc_new', keyboard
...
%      text(fix(length(Anout)/2.5),1.07+shi,stri);
      ...
      glosp=glosp;
      stri=[' gain/QW = ',num2str(glosp,'%0.4e'),',   Dlam = ',num2str(zel,'%0.4e')];
...
%      stri=[stri,' gain per QW = ',num2str(glosp/(NQW*fatqw),'%0.4e')];
      ...
      stri=['lam= ',num2str(laver,'%0.4e'),',     KMAX = ',num2str(alim,'%0.2f'),',  Nk = ',num2str(nK_dis,'%3i'),', N-acc = ',num2str(numodiacc,'%2i')];
%      stri=[stri,' gain per QW = ',num2str(glosp/(NQW*fatqw),'%0.4e')];
      text(-.25,1.08,stri);
      stri=' E_z  Output';
      %      stri=[stri,' gain per QW = ',num2str(glosp/(NQW*fatqw),'%0.4e')];
      text(55.25,0.98,stri);
      %'tedt', keyboard
      ax=axis;
      ax(2)=length(Anout+5);
      axis(ax)
...
if SPP==10   
save saK Afz Afzf Ef lambda rr KK YD ired_ret ldap1 XP YP X Y E2xo  -append
save saK  E2yo E2zo aax Cug E2xp E2yp xvero Pus pes -append
save saK Mvefm0 Mvefp0 Mvegm0 Mvegp0 Mvez0 Mvhz0 fkt -append
save saK besp besm Mvez besz mm pasnu X1 Y1 YD  -append
save saK mbv Pusas ldap Pured1 KKt rrfa KKv ipolar -append
save saK Kff teR iall numodi ibp1 ibm1 ibz1 Moe ibz2 Im npkf npkF gmu fmu fian -append
end
...
% 'qui Parametri fPdiff', keyboard
  Ef_sav=Ef;
% vecchio; il campo deve essere quello totale e qui invece uso solo quello progressivo
%  if i2D==3
%   Ec_at=Etb(:,:,2);
%  else
%   Ec_at=Etb(:,2);
...
% nuovo; in Etb ho ora la somma di progressivo e regressivo
  if i2D==3
   Ec_at=Etb(:,:,2);
  else
   Ec_at=Etb(:,2);
...
%  if iplan==1
   ...
   Npile=(N*(2*gg0-g0)+g0)/(2*gg0);
   if isrow(N)==1
      Npile=Npile';
   end
%   Nga=[Npile; zeros(length(x)-length(ro_in),1)];
   Nga=spline(ro_in,Npile,xvero)';
...
%' gam', keyboard
  ...
  Lsov(imod)=Glostotal*confz*vgconvPS;
%  keyboard
% 'step by step Lsov',  keyboard
%  keyboard
%  keyboard
%  keyboard
...
% end 
    ...
    Teff_old=(Int_b-Int_f)/Int_at;
%    Po=real(Poz);
    PointInt=real(xdx*Po*defi);
    Teff=rr/real(ra)*PointInt/Int_at;
    
%    'Teff', keyboard
   % Teff=rr*PointInt/Int_at;
    %real(ra) deriva dalla relazione fra DeltaEpsilon attivo e guadagno
    P_gain=ra*Glostotal*d*1e2;
    
% ieri qui: Teff o Teff1 circa P_gain: ma dove sono le altre perdite?
% fare plot 2D del vettore di Pointing da confrontare con quello in Flam_fieldCaricato2016Mod.m

Psez=squeeze(Poi(:,1,:));

PointInt=rr*real(xdx*Psez*2*pi)/Int_at;
Pord=[0 PointInt([1 2 4 3]) 0];
dd=diff(Pord);
Ploc=dd(1:2:5);

Teffi=rr/real(ra)*real(xdx*Psez*2*pi)/Int_at;


%Pqw=abs(Ploc(2));
Pqw=PointInt(2)-PointInt(4);

Prad=PointInt(1);
Controllo=P_gain/Pqw;
%keyboard

% figure, plot(Pord,'o'), 'Teff', keyboard
...
%    FF.Acoz=
   ...
   if isav_Az==1 & imod==0 & izo==1
    fr_sa=fre_camp;
    fre_camp=fr_sa-2*diff(fr_sa(1:2));
