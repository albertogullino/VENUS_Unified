...
% pack
 ...
 isomvero=0;
  if itutmir==0 & length(Tdis)>1
   L_pair=lambda/2/real(mean(n_i));
   isomv=isomvero+NP_sopra*L_pair;
 end
...
% keyboard
 ...
 if itutmir==0 & length(Tdis)>1
  L_pair=lambda/2/real(mean(n_i));
  isomv=isomvero+NP_sotto*L_pair;
%  isomv=isomvero;
%  'ferma', keyboard
 end
