...
%  ilinsu=[ilins(2:end)-1 length(Li)];
  ...
  ilins=[ilins length(Li)];
  Pstack.n=ni(1,1:end);
  Pstack.Li=Li;
  Fmlsi=fmlsi(:,2);
  Fplsi=fmlsi(:,2);
  for kli=1:length(ilins)-1
   Expol(kli)=1;
   if fmlsi(ilins(kli),1)~=1
    fm1=fmlsi(ilins(kli),1);
    fm2=fmlsi(ilins(kli),2);
    iin=ilins(kli);
    ifi=ilins(kli+1);
    pu=iin:ifi;
    fi=find(fmlsi(pu,1)==fm1 & fmlsi(pu,2)==fm2);
    Expol(kli)=fm2;
    Fplsi(pu(fi))=-Fmlsi(pu(fi));
    Fmlsi(pu(fi))=0;
    ilinsadd(kli)=pu(fi(end));
   end
  end 
  ilins=ilins_sv;
  if exist('ilinsadd')
   finz=find(ilinsadd>0);
  else
   finz=[];
  end 
  ilins_s=ilins;
  if length(finz)>0
   ilins=sort([ilins ilinsadd(finz(end))]);
  end 
% 'qui', keyboard
  ...
  if exist('ilinsadd')
  fili=find(ilinsadd~=0);
  for kf=fili(end)
   nsf=ilinsadd(kf);
   filii=find(ilinsu==nsf-1);
   ilinsu(filii)=nsf;
  end
  end
  Pstack.in=ilinsi; 
