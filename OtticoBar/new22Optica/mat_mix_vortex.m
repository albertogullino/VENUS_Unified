% 'salvato matmix vortex', keyboard
irid_bas=1;
%if ifp==-10
%'mat_mix vortex',  keyboard
%end
 KANzp=0;
 KANzm=0;
if ~exist('icomp_anymat')
 icomp_anymat=0;
end 
Madd=0;
shaC=0;
segem=-1;   %=-1 mette i segni come Love. =1 usa i segni TM cambiati, come fatto dall'inizio
%segem=1;   %=-1 mette i segni come Love. =1 usa i segni TM cambiati, come fatto dall'inizio
...
duse=find(real(nv(kseg,:)<0) & abs(imag(nv(kseg,:))<1));
 if length(duse)>0
  pros(kseg,1)=-duse(1);
  if duse>1 & duse<length(find(nv(kseg,:)~=0))
   pros(kseg,2)=-duse(1)-1;
  end
 else
  pros(kseg,1)=1;
 end
...
%  'icompanymat =1 ', keyboard
 ...
 orie=1;
 iscl=whos('par_grat');
 if strcmp(iscl.class,'struct')==0
  par_grat=radii.array{fish}{11};
 end
 ngra(1)=par_grat.n1; 
 ngra(2)=par_grat.n2; 
 tt=par_grat.per;
...
% 'matmix ', keyboard
 ...
 global igraef_new
  if igraef_new==0
%' sono qui in mat_mix igraef_new=0', keyboard 
%' sono qui in mat_mix', keyboard 
  ...
  anyret(fiseg)=2*Delta_eps;
  
%  'anyret cambiato', keyboard
 end 
...
%'qui mat_mix ingresso anyret', keyboard
end
...
fis0=find(radii.a(:,1)+radii.b(:,1)==0 & shavet(:,1)~=6);
sizsh=size(shavet);
...
racv=reshape(radii.a,prod(size(shavet)),1);
[shad,ipush]=sort(shavdu);
fino0=find(shad~=0 & dvcv(ipush)~=0  & racv(ipush)~=0);
...
%end  % prima era qui
 ...
 ity=0;
if length(find(shavet>4 & shavet~=7 & shavet~=6 & shavet~=11))>0
...
if length(find(shavet==11))>0
 inume=1;
end 

%'prima di rirety', keyboard
...
% ' saha ', keyboard
 fis=find(shai==sha);
if sha==1
 if length(find(shaiver==-8))>0
  fis=find(shai==sha | shaiver==-8);
 end
end
   if sha==7 | sha==4
    if sha==7
...
%disp(' mat_mix interno'), keyboard
  ...
  aytot(1:length(ayiA),shad)=ayiA;
  axtot(1:length(ayiA),shad)=axiA;
  pdtot(1:length(axiA),shad)=pdiA;
  
  if ity==1
   tytot(1:length(axiA),1)=tyiA;
  end
%disp(' mat_mix interno'), keyboard
...
%      disp(' mat_mix ver'),     keyboard
     ...
     DOE=1;
     fis=fis(1);
     adis=kcav*radii.array{ish(fis),1}{12};
     pgra=radii.array{ish(fis),1}{13};
     if isfield(pgra,'rDOE')
      rDOE=pgra.rDOE;
      adis=kcav*rDOE;
     end
...
%'key;,', keyboard
	...
	if isfield(Pf,'iDOE')==1
	 iDOE=Pf.iDOE;
	else
	 iDOE=0;
	end     	
     if igraef_new>=0 & iBWnew>=1 & iDOE==0
      sha=4;
      shape='sha_rhom';      
      bdis=adis*1.00001;
      pdi=zeros(size(adis));
     ...
     if iDOE==1      % procedo in modo diverso
      if iLP==1
       Kmat_LP
      else
      tic
...
%      'quie DOE 8', keyboard
      KOS8=sum(Kosp(:,:,1:2:end),3)-sum(Kosp(:,:,2:2:end),3);   
      KOSz8=sum(Koszp(:,:,1:2:end),3)-sum(Koszp(:,:,2:2:end),3);    
      MKosp{shad}=KOS8;               
      MKoszp{shad}=KOSz8;        
      KOS8=sum(Kosm(:,:,1:2:end),3)-sum(Kosm(:,:,2:2:end),3);   
      KOSz8=sum(Koszm(:,:,1:2:end),3)-sum(Koszm(:,:,2:2:end),3);    
      MKosm{shad}=KOS8;               
      MKoszm{shad}=KOSz8;      
%      map(KOS8)      
%     ' solo doe VERO '      , keyboard      
     if isfield(pgra,'RaggLente')
      rEXT=pgra.RaggLente;
      adis=kcav*rEXT;
      lxi=length(adis);
     
      aytot(1+[1:length(rEXT)],8)=rEXT;
%     shavet(ish(fis),1)=1;
%     aytot(:,1)=adis'/kcav0;
%     axtot(:,8)=0;      
      if iLP==1
...
%    keyboard
      ...
      KOS8=MKosp{shad};            
      KOSz8=MKoszp{shad};        
      KOS8(:,:,1+[1:length(rEXT)])=Kosp;
      KOSz8(:,:,1+[1:length(rEXT)])=Koszp;
      MKosp{shad}=KOS8;               
      MKoszp{shad}=KOSz8;        
        
      KOS8=MKosm{shad};               
      KOSz8=MKoszm{shad};        
      KOS8(:,:,1+[1:length(rEXT)])=Kosm;
      KOSz8(:,:,1+[1:length(rEXT)])=Koszm;      
      MKosm{shad}=KOS8;               
      MKoszm{shad}=KOSz8;         
%          ' solo doe EXT '      , keyboard      
     end
      ireturn=2;      
      sha=0;
     end
...
%    keyboard
   ...
   if shasav==8
    ...
    if (sha==6 | sha==5 | sha==3) & ifp~=-4 & ipolar~=0 & inume==0
...
%shailoop=abs(shailoop);
%'shailoopu', keyboard
shailoopud=shailoopu;
if ireturn==2 
 fi=find(shailoopu~=6);
 shailoopud=shailoopu(fi);
end
for sha=shailoopud
%  if sha==7
%   sha=1;
...
if (~exist('KAp') & iplan==0)
       fiatt=find(iauto(:,1)==2);
       shaat=shavet(fiatt,1);
       iposat=find(aytot(:,shaat)==aiat(1));
       KAp=MKosp{shaat}(:,:,iposat);    
       KAm=MKosm{shaat}(:,:,iposat);    
       KAzp=MKoszp{shaat}(:,:,iposat);    
       KAzm=MKoszm{shaat}(:,:,iposat);           
...
%     sT=ismat(T);
     ...
     if sT==2
      adis=xiT*kcav;
      if length(find(shavet(:,1)==4))>1
       ips=find(shavet(:,1)==4 & real(nv(:,2))<2 & abs(imag(nv(:,2)))<.01 ...
               & real(nv(:,2))>1);
...
%'% calcolo i coeff. di accop. della parte attiva', keyboard
%'quiattivo', keyboard
...
if ( (igau==0 & iplan==0) | igau==5 | (igau==4 & length(yiNd)<=1) )
...
%    disp(' errore 2 in mat_mix: manca KA! '), keyboard

if ifp~=-4 & ifr==1 & iplan==1
    map(KAp)
    title(' KAp ')
    pausak
   if iclo==1
            ...
            for kj=1:kjM
              ...
              if iscell(Mc)==0
                ...
                if iztm==1
                 ...
                 MKosp{ki}{kk}(Pus,Pus,kj)=MKospd{ki}{kk}(Pusasav,Pusasav,kj);
                 MKosm{ki}{kk}(Pus,Pus,kj)=MKosmd{ki}{kk}(Pusasav,Pusasav,kj);
                 if iztm==1
                    MKoszp{ki}{kk}(Pus,Pus,kj)=MKoszpd{ki}{kk}(Pusasav,Pusasav,kj);
                    MKoszm{ki}{kk}(Pus,Pus,kj)=MKoszmd{ki}{kk}(Pusasav,Pusasav,kj);
                 end             
                end 
            end
           end %kk 
...
%yprod=Tc;
if ifp==-5
 ' giochetto ifp in mat_mix fine', pausak
%' qui fine mat_mix', keyboard 
 ifp=-10;
end
%' fine KMIX Optica', keyboard
%if numodiacc<2
if ifp==-10
%' fine KMIX Optica', keyboard
end
