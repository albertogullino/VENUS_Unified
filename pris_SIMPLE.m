...
if mode.Pmin_Pfit>20
    ...
    figure(80),plot(mode.vv_dd(ibol),mode.ii_dd(ibol)*1e3,'k--','linewidth',2)
    figure(1),plot(mode.ii_dd(ibol)*1e3,mode.DeltaTmax(ibol),'m--','LineWidth',2)
    PPst=sum(mode.Pst_dd(:,ibol),1)+mode.Psp_dd(ibol);
    figure(81),plot(mode.ii_dd(ibol)*1e3,PPst,'k--','linewidth',2)
    
    figure(130),plot(mode.ii_dd(ibol)*1e3,mode.fattore_correttivo(ibol),'.-','linewidth',1)
    pausak
end
...
if mode.Oflg==1
    ...
    plot(mode.ii_dd*1e3,mode.fattore_correttivo,'.-','linewidth',1)
    xlabel('Current, mA'),ylabel('"Ignorance" factor')
end