...
elseif T0a==20
    Imassimo=18;  % massima corrente analizzata
    Vadd=[1.80:0.05:3.40];
end
...
% CN_Auger=0        % moltiplica 1e-30 in mw_phmat
...
if mode.quasi1D==1
    Exp_Temp0=-(1.1-0.0092*(T0-T300-20)) % 80C
%     Exp_Temp0=-1.1
...
ABS_Texp=1      % in VELM: ABS.eleccentro=ABS.eleccentro.*(1+Tvelm/T300).^ABS_Texp;
...
mode.minthermalvoltage=0.5; % min. voltage to activate thermic simulator
...
if T0a==20
    mode.Imin=2e-3/mode.CarrierNorm;  % 1.5 in case of "etched" TJ
elseif T0a==50
    mode.Imin=3.5e-3/mode.CarrierNorm;  % 1.5 in case of "etched" TJ
elseif T0a==80
    mode.Imin=3e-3/mode.CarrierNorm;  % 1.5 in case of "etched" TJ
elseif T0a==110
    mode.Imin=2.5e-3/mode.CarrierNorm;  % 1.5 in case of "etched" TJ
    mode.IstepLarge=mode.IstepLarge/2; % A, current step for current driving above Imin
end
