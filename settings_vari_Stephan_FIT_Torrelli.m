...
mode.ImoveON=1.5; % current threshold to disable mode.MoveON
mode.MAXiterMoveOn=8; % max iteration when mode.MoveON is enabled
mode.Imax=10; % maximum current which reaches "before roll off"
mode.TQWFake=0; 
mode.Elementi=1; 
mode.ThermalDB=0; 
...
tauH=5/10;         % hole capture time, ps
...
CN_Auger=.5;
FatNP_Auger=0.7;     % questo lo tratto come un fattore, vedi mw_phmat
CTemp_Auger=0.5;
%CTemp_Auger=2;
FatAuger23D=0.7;
C_Temp_DD=1;
...
% mode.FatQcontact=0.65; % scaling at the contact (0.65 for Oxide; 1.00 for TJ)
mode.FatQcontact=0.90; % scaling at the contact (0.65 for Oxide; 1.00 for TJ)

% fCondTer=1;   % transverse thermal conducibility % original -> toccare questo qui! Abbassarlo un po' sperando che scaldi un po' di più!
fCondTer=0.90;   % transverse thermal conducibility % original -> toccare questo qui! Abbassarlo un po' sperando che scaldi un po' di più!
fCondTerZ=0.8;   % Longitudinal thermal conducibility % original
% fCondTer=0.8;   % transverse thermal conducibility
...
if mode.quasi1D==1
    ...
    % Exp_Temp0=-1.30;	% VENUS -> DA CAMBIARE DA -1.3 a -1
    Exp_Temp0=-1;	% VENUS -> DA CAMBIARE DA -1.3 a -1
    % fatt_dndT=1.2195;  % dn/dT  2.3e-4 da dati sperimentali
    % fatt_dndT=1.217;  % con il vecchio raggio termico (piccolo)
    fatt_dndT=1.257;  % con il nuovo raggio termico (grande)
end
TARde=1;
mode.ABSe=5;
mode.ABSh=11;
...
ExpH=0.7;       % exponent for thermal dependence of hole mobility
...
if mode.flgBTJ==1
NUMERO_MODI_v=[1 1 1 2 3 4 4];
else
NUMERO_MODI_v=[1 1 1 3 4 4 4];
end
NUM_Azim_v=[1 1 1 2 3 3 3];

NUMERO_MODI_v=[1 1 1 1 1 1 1];	% modi azimutali
NUM_Azim_v=[1 1 1 1 1 1 1]; 	% numero modi in VENUS
...
NP_k=[30 30 25 25 95 20 20 20];
NP_k=[30 30 25 25 25 20 20 20];
...
%     VelmOptions.Dlam=[-.2 2.5 10 0 .4]; % Modified for Stephan at high temperature!
...
mode.minthermalvoltage=1.6; % min. voltage to activate thermic simulator
%mode.mintempVELM=1e-8; % degrees, minimum temperature such that VELM is called
mode.IsoThermal=0;
...
VelmOptions.igraef_new=0;
VelmOptions.iBWnew=1;   %0 BW, 1 f_Nefficace
VelmOptions.iany=3;  %necessario per anisotropia finita
if iStruttura==27
VelmOptions.iany=0;  %necessario per anisotropia finita
end
...
mode.stats='Boltzmann'; % "Fermi" or "Boltzmann" are available
